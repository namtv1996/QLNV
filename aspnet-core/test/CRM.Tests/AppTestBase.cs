...
namespace CRM.Tests
{
    ...
    /// Provides methods to easily work with <see cref="CRMDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<CRMTestModule>
    {
        ...
        {
            void NormalizeDbContext(CRMDbContext context)
            {
        ...
        protected void UsingDbContext(Action<CRMDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<CRMDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CRMDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CRMDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CRMDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CRMDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<CRMDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CRMDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CRMDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CRMDbContext, Task<T>> func)
        {
            T result;
